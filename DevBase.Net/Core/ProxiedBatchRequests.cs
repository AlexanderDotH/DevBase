...
public sealed class RandomStrategy : IProxyRotationStrategy
{
    ...
    public TrackedProxyInfo? SelectProxy(List<TrackedProxyInfo> proxies, ref int currentIndex)
    {
        var available = proxies.Where(p => p.IsAvailable()).ToList();
...
public sealed class LeastFailuresStrategy : IProxyRotationStrategy
{
    public TrackedProxyInfo? SelectProxy(List<TrackedProxyInfo> proxies, ref int currentIndex)
    {
        ...
        var selected = available.OrderBy(p => p.FailureCount).ThenBy(p => p.TotalTimeouts).First();
        currentIndex = proxies.IndexOf(selected);
        return selected;
    }
}
...
public sealed class StickyStrategy : IProxyRotationStrategy
{
    public TrackedProxyInfo? SelectProxy(List<TrackedProxyInfo> proxies, ref int currentIndex)
    {
        ...
        {
            var current = proxies[currentIndex];
