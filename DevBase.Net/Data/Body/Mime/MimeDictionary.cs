...
namespace DevBase.Net.Data.Body.Mime;
...
public class MimeDictionary
{
    ...
    private static readonly FrozenDictionary<string, ReadOnlyMemory<char>> MimeTypes = CreateMimeDictionary();

    private static FrozenDictionary<string, ReadOnlyMemory<char>> CreateMimeDictionary()
    {
        Dictionary<string, ReadOnlyMemory<char>> dict = new Dictionary<string, ReadOnlyMemory<char>>
        {
            { "*", "*/*".AsMemory() },
    ...
    {
        return MimeTypes.TryGetValue(FilterQuery(mimeType), out mimeResult);
    }

    public bool TryGetMimeTypeAsSpan(string mimeType, out ReadOnlySpan<char> mimeResult)
    {
        // Use AlternateLookup to avoid string allocation in FilterQuery if possible, 
        // but FilterQuery(string) returns string. 
        // We can optimize FilterQuery to return ReadOnlySpan<char> and use AlternateLookup.
        
        ReadOnlySpan<char> querySpan = FilterQuerySpan(mimeType.AsSpan());
        if (MimeTypes.GetAlternateLookup<ReadOnlySpan<char>>().TryGetValue(querySpan, out ReadOnlyMemory<char> memory))
        {
            mimeResult = memory.Span;
            return true;
        }
    ...
    {
        if (TryGetMimeTypeAsMemory(mimeType, out ReadOnlyMemory<char> result))
            return result;
    ...
    {
        if (TryGetMimeTypeAsSpan(mimeType, out ReadOnlySpan<char> result))
            return result;
    ...
    private static ReadOnlySpan<char> FilterQuerySpan(ReadOnlySpan<char> query)
    {
        if (query.Length > 0 && query[0] == '.')
